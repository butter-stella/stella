## Authority-bound validation/apply envelope for one canonical BGM operation.
class_name BgmOperationRequest extends RefCounted
...
func _apply(
	...
	if (
		...
		or not BgmChannelState.validate_snapshot_state(committed_state, false)
	):
		return false
