...
func _apply_volumes():
	...
	var se_vol = NatsumeRuntime.get_setting("se_volume")
	if se_vol == null:
		se_vol = 1.0
	for player in _se_players:
		player.volume_db = _to_db(master * se_vol)

	var voice_vol = NatsumeRuntime.get_setting("voice_volume")
	if voice_vol == null:
		voice_vol = 1.0
	_voice_player.volume_db = _to_db(master * voice_vol)

	var sys_se_vol = NatsumeRuntime.get_setting("system_se_volume")
	if sys_se_vol == null:
		sys_se_vol = 1.0
...
func _on_bgm_play(asset: String, fade_duration: float):
	var stream = _load_audio(NatsumeRuntime.bgm_path, asset, ["ogg", "mp3"])
	if stream == null:
...
func _on_se_play(asset: String, _loop: bool):
	var stream = _load_audio(NatsumeRuntime.se_path, asset, ["ogg", "wav"])
	if stream == null:
...
func _on_voice_play(asset: String, character: String = ""):
	...
	var stream = _load_audio(NatsumeRuntime.voice_path, asset, ["ogg", "wav"])
	if stream == null:
		push_warning("AudioPresenter: Voice not found: %s" % asset)
		return

	# Check per-character mute
	var char_enabled = NatsumeRuntime.get_setting("character_voice_enabled")
	...
	var char_vol = NatsumeRuntime.get_setting("character_voice_volume")
...
func _on_choice_selected(_option_id: String):
	var select_se = NatsumeRuntime.config.se_select
	if select_se != "":
