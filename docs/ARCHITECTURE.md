...
#### Marker-synchronized BGM native boundary

Issue #190 明确引入一个最小 C++/godot-cpp GDExtension 设计决定；这也是当前仓库“不默认引入 native code”规则的唯一局部例外，不建立 Rust、通用 native framework 或第二 composition root。Godot 4.6 的 GDScript/`AudioStreamSynchronized` public gain API 必须经过 Variant/engine synchronization，不能在 source sample H 对全部 stem 做 audio-thread atomic switch；`AudioStreamPlaybackResampled` 又会在扩展 callback 之前预解码自己的固定 chunk，使 outer callback admission boundary 不再可见。因此 marker-capable track 使用一个 direct custom `AudioStreamPlayback`：它解码全部 OGG stem、做 fixed-point linear source-rate interpolation，并在同一 cursor/loop 内完成 sample split 与 gain ramp。普通无 marker BGM 仍走既有 `AudioStreamSynchronized` OGG/MP3/WAV 路径。

public resource/API 仍是 typed `BgmTrackDefinition.markers: Array[BgmMarkerDefinition]`，其中 marker 是 exact Unicode label + sample frame；native `configure(Dictionary)` 只是 AudioPresenter main-thread preflight 的 strict closed-schema FFI。Godot 导出不会保留 source `.ogg` raw bytes，所以 Presenter 从 imported/export-safe `OggPacketSequence` 确定性重建 Ogg pages，然后一次性把 immutable bytes、ordered markers、loop frames 与 initial gains 交给 stream。track 与 marker-table fingerprint 都以显式 schema prefix、little-endian fixed-width fields、UTF-8 与 SHA-256 编码；不能使用 Godot process-local `hash()` 作为 restore identity。

Audio callback 是 arm 的唯一线性化点：它在 callback 入口消费 fixed POD command，按 earliest-not-yet-activated `(source frame, loop epoch)` horizon 选择第一个 `frame >= horizon` 的 matching authored occurrence，并绑定 exact target loop epoch。raw per-stem prefetch 使尚未输出的 H 不会被 decoder head 跳过，H 命中 H，H+1 选择后一个 occurrence；即使 rate>1 让单次 callback 跨过多个短 loop epoch，内部仍逐 source frame 调用 `activate_source_frame()`，所以会在首个 exact `(frame, epoch)` 命中而不是用 bool wrap 推测。callback 在 H 切分并开始 source-frame ramp。它不构造 Variant/Array/Ref、不 allocate/lock/call_deferred/emit signal，也不调用 resource API；每 stem 使用预分配 chunk decode，而非 per-source-sample decoder call。command ring admission 预留该 command 最坏 `armed/triggered/completed` terminal credits，因此 event ring 不依赖可覆盖 sticky slot；replacement 在同一 callback transaction validate 后才 swap，失败保留 old arm/receipt。Skip/cancel cut 同样预留一个 `CUT_APPLIED` credit，callback 应用 physical gains 后 Presenter 才更新 canonical state并 settle receipt。main thread 上只有 Runtime-owned `AudioPresenter._process()` drain 已时间戳的 POD event ring并投递 exact terminal，它不决定时间或 marker，因而不是 scheduler/polling transport。scene/reset teardown 先增加 Presenter/native generation、清 arm 与 receipt，旧 event 即使迟到也不能跨 owner 生效。debug `rt_*_violations` 只是 forbidden fallback sentinel，不是通用 allocator/mutex interposition；fixed storage code audit、warning-as-error build、32-stem chunk-refill bound 与 callback tests共同证明该边界。

`BgmPendingMarkerMixState` 是贯穿 Presenter、`PresentationState` 与 save 的 typed value；JSON 边界才序列化为 versioned closed-schema Dictionary。enqueue 成功立即发布 coherent `QUEUED` descriptor，audio ack 后成为绑定 exact frame/ordinal/loop epoch 的 `ARMED`；native callback-wide generation 在整个 `_mix()` 期间为 odd，尾部发布 horizon 后才回到 even，因此 capture 不会把 in-flight callback 的旧 horizon 与新 enqueue command 拼接。稳定 snapshot 同时包含 command sequence、active arm，以及唯一的 earliest-not-yet-activated `(source frame, loop epoch)`。`position` 在 pending 状态下投影自同一个 source frame，而不是另一个 resampler playback cursor；因此 fractional prefetch、相邻同名 occurrence 与 loop wrap 都不会在 restore 时改选。save 在 enqueue-before-ack、armed 或 trigger split 任一窗口都有唯一快照：split 已触发则保存 final mix，未触发则保存 QUEUED/ARMED 与 exact restore horizon。restore 先核验 marker + track fingerprints，再把 exact horizon/target 写入 custom stream 的 startup descriptor；playback `_start()` 在首个 audio callback 前发布 command，并以 atomic silent full-buffer hold 覆盖 `player.play()` 到 Presenter 登记 owner/paused 状态的窗口。hold 期间 callback 返回 requested frame count 的 exact zero PCM，不推进 decoder/cursor/command/event/gain/ramp，因而 AudioServer 不会把 playback 当作 short-buffer end。Godot JSON decode 会把数字表示为 float；SaveManager 只在 disk boundary 对 pending closed schema 的六个 integer field 做 finite、integral、JSON-safe-range 检查后转回 int，runtime typed boundary 仍拒绝 float/String/Bool。旧存档只允许缺少新增 `pending_marker_mix` 时 clean-default `{}`，字段存在但 malformed/unknown version 一律在 provider mutation 前 fail-close。rollback/load/restart/skip/abort/same-target re-dispatch 继续使用 Director receipt/generation：旧 receipt exact superseded/cancelled/completed，new receipt 可 rebind 同一 native arm，stale callback 永不取得新 state。

extension 使用 C++17、Godot 4.6 API，并将 godot-cpp commit/ABI 与 stb_vorbis source/license 固定在 `native/marker_bgm`。debug hooks 只在 `DEBUG_ENABLED` 绑定；release public surface没有 allocation/lock counters。仓库只跟踪 descriptor source `addons/stella/native/stella_marker_bgm.gdextension.in` 与可重现 build 配置；active `.gdextension` 和 binary 都在 build 后生成并保持 ignored。CI 从 clean checkout 构建 macOS universal（arm64+x86_64）、Linux x86_64、Windows x86_64 的 debug/release，export/PCK smoke 会真实 instantiate/configure/decode PCM，并把生成的 descriptor+binary 作为 per-platform artifact 发布。新增平台必须先增加可重现 binary build + exported-process load probe，不得静默立即切 mix。这个决定带来每个 Godot/godot-cpp ABI 升级都要重编两种 template、维护 compiler matrix 与 third-party notices 的发布成本。
...
BGM 使用同一 `PresentationState` 中的单一 `bgm` stable Dictionary：stopped 为 `{}`，active 精确为 `{asset, cue, loop, pending_marker_mix, position, status, stem_mix, volume}`，其中 status 只允许 `playing` / `paused`；single-stream 的 `stem_mix` 与 `pending_marker_mix` 都必须是 `{}`，multi-stem 保存完整 name→gain。普通 save capture 采样 canonical incoming player 的当前 cursor；marker-capable voice 则由同一 AudioPresenter 把 native seqlock snapshot 与 typed `BgmPendingMarkerMixState` 合成为一个 coherent state：enqueue-before-ack 是 `queued`，audio boundary ack 后是 exact frame/ordinal/loop epoch 的 `armed`，二者都保存 exact restore horizon，H split 后是 final mix且 pending 清空。crossfade/mix Tween progress、receipt 与 wall-clock 不入档。pause fade 中保存会采样当下 cursor，但恢复直接投影稳定 paused；crossfade 中保存只投影 incoming target/current cursor。这是有意的 cut projection，不猜测旧 Tween 进度。

恢复先在 `SaveManager` / Runtime transaction preflight 同步验证完整 track/stem/loop/marker signature，并让唯一 AudioPresenter side-effect-free 验证 saved exact occurrence 仍可达；任一 mismatch 都在 choice、其他 provider、scene 或 player mutation 前 fail-close。成功后无 marker 的 multi-stem 仍以一个 `AudioStreamPlayer(AudioStreamSynchronized)` cut-project；marker-capable multi-stem 以一个 `AudioStreamPlayer(StellaMarkerBgmStream)` 重建 direct playback 并 re-arm exact occurrence。两条路径都只有一个 player/cursor；每个 stem 相位对齐，settings 只乘 player 总 gain。live same-resource 原地复用要求 ordered stem/default/source/marker signature 精确不变。旧存档仅允许缺少新增 `pending_marker_mix` 字段并 clean-default `{}`；malformed/unknown pending version fail-close。旧 String `bgm` snapshot 不是版本化公共 schema，没有第二套 runtime compatibility handler。
...
- **BGM**：单一 `bgm:main` typed channel；play/mix/pause/resume/stop 的 authored fade、最多 incoming+outgoing 两个 player 的整段 track crossfade、稳定 paused cursor、resource/named-cue loop region、exact receipt/generation、save/restore cut projection。原始 OGG/MP3/WAV 默认循环到 stream end并保留 native marker；`BgmTrackDefinition` 是 single stream / 2..32 synchronized stems 的严格 sum schema，并提供完整 default 与不继承的 named `BgmCueDefinition`。definition 的 `loop_end_position=-1.0` 解析为 physical end；显式 end 仅写 duplicate：WAV 写 sample loop end，OGG/MP3 写 mixer beat boundary并校验小于一 source sample的 round-trip error。无 authored marker 的 multi-stem 使用一个 `AudioStreamSynchronized` player；有 marker 的 imported OGG stems 使用一个 direct `StellaMarkerBgmStream` playback，在 audio callback 入口按 earliest-not-yet-activated exact `(source frame, loop epoch)` 选 occurrence、按 marker frame split、原子应用完整 gains；rate>1 时 callback 内仍逐个 activate 中间 source frame，target arm 绑定 exact loop epoch而不是 bool wrap。persistent Skip 的 force-cut intent 在 `BgmOperationRequest` 构造时即封存，Presenter preflight 仍验证 sealed resource、stem mix、marker table 和 native command credit；通过后 JOIN/FNF 都走同一 immediate native command，在下一个 audio callback 应用完整 target，并由 `CUT_APPLIED` 作为唯一 terminal。它不会因为 occurrence 已位于 cursor 后方而失败，也不会跳过不存在 label 的 source-located 诊断。source step 在 control/command consume 前完整校验；正数但过小而不能表示 fixed-point increment、或超过受支持上限的 speed scale，会输出 requested 长度的 exact silence并保持 command/cursor/gain/event 不变，恢复正常速率后同一 command 才在下个 callback boundary 执行。只有 stop、non-loop EOF 或 decoder terminal 才允许 short buffer，且 terminal 会完成已触发 ramp或失败未到达 arm，不遗留 receipt。它用预分配 chunk decoder 与 fixed POD command/event credit rings；唯一 AudioPresenter 的 `_process()` 只 drain 已确定事件，不计时、不选 marker。没有 per-stem player、第二 thread/scheduler、Variant/deferred call、seek/restart 或 wall-clock poll。`rt_*_violations` 是 forbidden fallback path 的 sentinel，不是通用 allocator/mutex interposition；零 callback allocation/lock 的证明来自 fixed storage/无动态 RT API 的 code audit、32-stem chunk-refill bound 与 callback contract tests共同约束。
