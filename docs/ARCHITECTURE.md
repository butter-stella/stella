...
Core 的 `TextResourceInspector.inspect()` 是源码 `.tscn` / `.tres` 无副作用预检的单一 typed API；`StellaRuntime` 只消费 `InspectionResult`，不再承载资源 grammar。Inspector 会在任何 Godot resource parser 调用前校验 tag 与 attribute key 的 identifier token、node/sub-resource 类型是否存在并继承正确的 `Node` / `Resource` 基类（包括注册到 ClassDB 的 GDExtension class），并把 quoted/numeric resource ID 规范化到同一无歧义 key，因此 Godot 4.6 format 2/3 的 numeric ID 保持兼容。它还递归合并 inherited root 与 nested PackedScene 的 effective node tree，验证每个 parent、owner、connection endpoint 和 editable path；property-only override 只有在完整 NodePath 已由 inherited/nested tree 提供时才合法。
...
Compressed PCK 的真实进程回归同时覆盖 numeric resource ID，以及 unknown native type、无效 parent/owner/attribute 的 title、game 与 overlay 目标；失败路径断言旧 scene/context/overlay/state owner 不变，外层日志 gate 则确认私有 sentinel 没有进入 Godot parser。
