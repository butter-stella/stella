...
`StellaRuntime` 作为唯一 composition root，只构造并持有一个 `PresentationDirector`。Stage、addressable dialogue avatar、dialogue visibility、dialogue content clear、chapter indicator、persistent loop-SE 与 BGM 都通过这个 owner/lifecycle 边界：

- `PresentationOperation` 是只读 typed operation，暴露 kind、channel、deep-copy payload 和 source location。concrete kind 为 `StagePresentationOperation`（`stage:<layer_id>` / `stage:*`）、固定 channel `dialogue:avatar` 的 `DialogueAvatarPresentationOperation`、`DialogueVisibilityPresentationOperation`（`dialogue:surface` / `dialogue:quick_menu`）、固定 channel `dialogue:content` 的 `DialogueClearPresentationOperation`、固定 channel `chapter:indicator` 的 `ChapterIndicatorPresentationOperation`、`LoopSePresentationOperation`（`loop_se:<channel_id>`），以及固定 channel `bgm:main` 的 `BgmPresentationOperation`。
...
Core → Presentation 的 canonical 对话与语音链均使用只读 typed DTO。集合 getter 返回 defensive copy；同步 `VoicePlaybackRequest` 携带 asset、character、owner validator、可选 DSP logical preset 与 source location，接受/拒绝结果由 `SignalBus` 持有的 `VoicePlaybackResponse` 返回，accepted playback 的完成状态则通过只读 `VoicePlaybackCompletion` 观察。物理与逻辑语音分别使用 `VoicePlaybackEvent` / `DialogueVoicePlaybackEvent`；旧 `voice_*`、`dialogue_voice_*` 信号只作为兼容 notification adapter，不承载 DSP selection 或内建状态。Backlog 记录使用 `DialogueRequest.entry_id` 定位 enrichment，不重新读取可变的 ScenarioContext 当前命令或 cursor。
...
- **Voice**：对话同步，角色独立音量控制；语音未播完（包括显式 effect tail）可阻止自动推进。每个 segment 可用 `#voice_dsp:<logical-id>` 选择 `VoiceDspChainDefinition`。AudioPresenter 在任何 live voice/private bus 变更前 detached-load 并在不可达的 staging bus 上严格安装、验证整条有序 chain；pre-commit staging 失败按 exact bus name 释放，旧 token/stream/bus/effects 不变，缺失/非法 preset 绝不回退干声。旧 voice 的 FINISHED 是同步重入边界：若 listener 已建立 replacement，外层只释放自己的 staging，不覆盖 replacement。commit 先把 player 切到新 bus，再按旧 bus name 释放，因此 AudioServer index shift 不会误删新链。accepted voice 的 player 固定 unity gain，Presenter-owned private bus 是 master/voice/per-character/enabled 的唯一 post-effect gain authority，所以 active source 与已缓冲 tail 同步服从设置且不双重增益。band-pass 以有序 HighPass+LowPass 实现，delay 的 `mix` 是不衰减 dry=1.0 的 wet tap linear gain。自然 stream end 后唯一 child Timer 持有 `tail_seconds`；替换、owned hide/clear、abort、load/rollback、return-to-title、quiesce 和 tree exit exact 取消 Timer、token 与 effects。既有 unowned dry playback 可保持其公开物理生命周期，但 unowned processed playback 在同一硬边界整体退休，绝不会让同一 token 中途变回干声。Backlog 保存每个 segment 的 canonical preset 并通过同一 typed request 回放；普通 dry voice 和不同 preset 都原子替换旧 chain。该 Stella/Godot primitive 只承诺公开参数与经真实 AudioServer mix 验证的确定性频响，不宣称与任意外部 Butterworth 实现逐 sample 等价；Godot `AudioEffectBandPassFilter.resonance` 没有文档化 bandwidth 映射，因此不用于这一 contract。
