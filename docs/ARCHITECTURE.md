...
解析不是在旧对象上反复打补丁。`_load_project_config()` 每次都从新的 `StellaConfig` 和内置默认值开始，`_apply_config()` 则无条件完整复制 resolved snapshot（即使没有来源成功应用），因此删除或禁用配置来源后重新解析不会让 Runtime 镜像残留上一次的路径或其他值；需要复用配置对象的调用方可用 `StellaConfig.reset()` 恢复默认值，并同时清空 `has_config_file`、来源列表和错误元数据。完成解析和应用后才创建 Core 子系统与全局 Presenter；项目默认主场景是无 UI 的 `bootstrap.tscn`，它随后按最终 `title_scene` 进入首次场景。标题 resolver 会递归预检 PackedScene 的全部外部依赖和嵌套实例，并在状态提交前拒绝不存在、声明类型与实际资源不兼容、现存但不可解析、依赖退化、无节点、含 bootstrap 行为、脚本原生基类与节点类型不兼容、脚本不可实例化或有效 `_init()` 仍需实参的候选场景，统一回退内置标题。源码 `.tscn`/`.tres` 依赖先由脱敏 metadata parser 校验，不把 malformed 私有值交给 Godot 会回显原文的 resource parser；导出后的 remapped/binary 资源则读取结构化依赖元数据。继承场景按最终 effective root script override 校验，仍继续检查未被覆盖的 base children。首次启动和之后返回标题使用同一个 resolver；`return_to_title()` 会延迟整个切换事务以避开场景根 `_ready()` 的 busy-parent 阶段，并且只在 `SceneTree.scene_changed` 后确认最终 `current_scene` 是目标标题，才清理运行状态、切到 `TITLE` 和播放标题 BGM。SceneTree 最终仍拒绝已预检的自定义场景时，返回路径会再尝试内置 fallback。已有项目若使用自定义主场景，插件不会覆盖；旧版内置标题主场景会迁移到 bootstrap。场景内 Presenter 在此之后构造或进入 ready，不会观察到部分应用的配置。

标题依赖和 Runtime scene destination 带 UID 时优先跟随 UID registry 的当前规范路径，只有依赖 UID 不可用才采用序列化 fallback，避免资源移动后被旧 fallback 误判为缺失，也避免以 `uid://` 请求后拿 canonical `res://` 比较而永久占用导航槽。所有 Runtime 场景入口（包括 legacy `start_scenario`）共享单一 navigation generation 和在途场景请求槽：每次先无副作用地解析 scenario/save 并加载目标 PackedScene；校验失败不会取得所有权，也不会改变当前 scene、context、overlay、state 或路径。有效的后调用会接管所有权，使旧 ScenarioEngine run generation 同步失效，等待旧场景请求落地后再发出自己的目标请求；只有确认最终 scene 后才脱离旧 context、发 abort 并提交新运行状态。旧 continuation 每次 await 后必须验证所有权，因此最终 destination 与状态遵循 last-call-wins。
